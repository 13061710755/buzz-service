...
let selectSchedules = function () {
    return knex('users')
        ...
        .select('users.user_id as user_id', 'companion_class_schedule.status as status', 'companion_class_schedule.start_time as start_time', 'companion_class_schedule.end_time as end_time');
};

let checkTimeConflictsWithDB = async function (user_id, time, start_time, end_time) {
    let selected = await knex('companion_class_schedule')
...
const create = async ctx => {
    ...
    try {
        ...
        let inserted = await knex('companion_class_schedule')
